...
#################### modified by J.Signell, Princeton 2015 #####################
################################################################################
...
compute_volSk <- function(DSD,tabD){
    ...
    idw   <- which(rS>0)
    Nw    <- length(idw)
    tabm  <- rep(NA_real_,nrow)
    if(Nw>0){tabm[idw] <- rowSums(DSD[idw,]*D[idw,])/rS[idw]}
    M <- matrix(rep(tabm,NtabD),nrow=nrow,ncol=NtabD)
    tabm2 <- rep(NA_real_,nrow)
    tabm3 <- rep(NA_real_,nrow)
    tabSk <- rep(NA_real_,nrow)
    if(Nw>0){
...
compute_gamma.R <- function(tab.mu,tab.lam,tab.Nt){

    ## Computes the rain rates associated to Gamma DSD parameters
    ## The Gamma DSD model is given by N(D) = alpha*Nt*D^mu*exp(-lam*D)

    ## Inputs:
    ## tab.mu  = vector of mu values [-]
    ## tab.lam = vector of lambda values [1/mm]
    ## tab.Nt  = vector of Nt values [1/m^3]
    ...
    if(Nmu!=Nlam){stop("dimensions of mu and lam do not match")}
    if(Nmu!=NNt){stop("dimensions of mu and Nt do not match")}
    if(Nmu==0){return(c())}

    if(any(tab.mu<=(-1),na.rm=TRUE)){warning("some mu values were smaller than -1")}
    if(any(tab.lam<=0,na.rm=TRUE)){warning("some lam values were negative")}
    if(any(tab.Nt<0,na.rm=TRUE)){warning("some Nt values were negative")}

    tabR <- rep(NA_real_,Nmu)
    idd  <- which(tab.Nt==0)
    tabR[idd] <- 0

    select <- intersect(intersect(which(tab.mu>(-1)),which(tab.lam>0)),which(tab.Nt>0))
...
compute_gamma.A <- function(tab_mu,tab_lam,tab_Nt,seqD,seqExt){
    ...
    if(any(seqD<=0,na.rm=TRUE)){stop("the values in seqD must be strictly positive")}
    if(any(seqExt<=0,na.rm=TRUE)){stop("the values in seqExt must be strictly positive")}
...
##### Additional scripts for inspecting DSD and calculating summary info #####

get.campbellsci_file <- function(path){
  ## Reads the contents of a standard campbellsci file and throws out record number
  
  ## Input:
  ## path = relative or absoulte path to file
  
  ## Output:
  ## file = data.frame with rownames as timestamp stings

  all_content <- readLines(path)
  skip_second <- all_content[-1][-(2:3)]
  file <- read.csv(textConnection(skip_second), header=TRUE, stringsAsFactors=FALSE)
  rownames(file) <- file$TIMESTAMP
  file <- file[ -c(1:2) ]
  
  return(file)
}

get.DSD<- function(file){
  
  ## Returns a 32x2 matrix containing the lower/upper velocity limits (in m/s)
  ## of the Parsivel disdrometer.
  
  ## Inputs:
  ## path = path to campbellsci TOA5 formatted DSD data, with 4 rows of header
  ##        data, the second of which contains the column names
  ...
  DSD <- sweep(intercept,MARGIN=2,w,`*`)
  return(DSD)
}
...
plot.DSD <- function(tabN, save_path='./'){
  ...
  jpeg(file = paste0('DSD_', toString(as.Date(tabN$date_time[1])),'.jpg'))
  # plot heat map with linear scale
  p <- ggplot(data=storm.m, aes(x=date_time, y=drop_diameter, fill=drops)) +
    geom_tile(aes(height=width))+
    scale_fill_gradient(low = "white", high = "darkblue") +
    ylab("Drop Diameter (mm)")+
    ylim(0.2495,2.0)+
    ggtitle(paste(c("Drop Size Distribution during",
                    toString(as.Date(tabN$date_time[1])),
                    "storm\n"), collapse = " "))+
    theme(plot.title = element_text( face="bold"))
  file = paste0('./output/DSD_', toString(as.Date(tabN$date_time[1])),'.png')
  ggsave(file)
  return(p)
}

get.sum_stats<- function(DSD, tabD, tabN){
  ## Return summary stats on drop size distribution for a particular storm
  ...
  mean_diameter <- compute_volmD(DSD,tabD)
  median_diameter <- compute_volD0(DSD, tabD)
  
  std_dev = rep(NA_real_, dim(DSD)[1])
  for(i in 1:dim(tabN)[1]){
    sqdiff <- (mean_diameter[i]-tabD)^2
    std_dev[i] <- (sum(sqdiff*tabN[i,1:32])/sum(tabN[i,1:32]))^(0.5)
  }
...
plot.rate <- function(data, save_path){
  ## Generate a plot of rain rate
  
  ## Inputs:
  ## data = data.frame of interesting computed values for each timestep
  
  ## Output:
  ## p = ggplot2 dependant plot using of rain rate
  
  library(ggplot2)
  p <- qplot(data=data, date_time, rain_rate, geom="line", color= I("red"), 
             ylab="Rain Rate (mm/hr)",
             main = "For entire time series") +
        ggtitle(paste(c("Parsivel Rain Rate during",
                        #toString(as.Date(data$date_time[1])),
                        "storm\n"), collapse = " "))+
        theme(plot.title = element_text( face="bold"))
  return(p)
}
